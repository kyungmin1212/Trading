...
- 결과
    ...
    - 수수료가 0.04% 인데 슬리피지를 고려하여 0.05% , 포지션 오픈 클로즈를 모두 고려하면 한번 오픈 클로즈할때마다 0.1%의 수수료 발생 따라서 전액을 모두 사용하여 매매를 한다고 가정했을때 balance 대비 수수료를 제외한다면 0.99퍼가 계속해서 남게될텐데 복리로 인하여 100번만 거래해도 0.99^(100)=0.366 으로 원금대비 36프로밖에 남아있지 않게 됨
    - 따라서 balance를 기준으로 매매
