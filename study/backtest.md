...
- 수익구조를 제대로 알아야함.
    - 한번 50% 손실을 보게된다면 다시 그만큼을 따기위해서는 200% 수익을 봐야함.
...
### RSI Divergence를 이용한 매매 기법(Long)

- 타임프레임 : 5분봉
- 매매 기법 
    - rsi 가 특정 값 이하에 존재하는 값들을 이어서 rsi 는 상승하는데 주가가 하락한다면 매수 시그널
    - rsi가 왼쪽 오른쪽 기준 특정 봉 개수에서 가장 작은 값을 나타내는 봉을 찾고 작은 값들의 봉들을 이어서 만약 rsi 값은 증가하는데 주가가 하락한다면 상승 시그널로 보고 매수 진행(단 두 시그널 사이간에 너무 많은 봉이 존재하면 그것은 다이버전스를 의미하는 가치가 없어짐. 따라서 특정값 이하의 rsi 값들 사이의 봉의 개수를 제한을 두어 테스트하기)
    - rsi 를 구할때 저가를 이용하여 구해주기. 또한 가격이 하락했는지 상승했는지 여부도 저가를 이용해서 테스트.
- 단방향 매매 설정 (추후에 하락 장에서도 사용할수 있도록 short 포지션에서도 테스트 해보기)
- 사용 지표 : RSI
- 변수의 수 : 8개
    - RSI Lower Limit
    - RSI period
    ...
    - 최대 포지션 open 개수 : n개
- 코드
    - https://github.com/kyungmin1212/Trading/blob/main/code/rsi_long_divergence_backtest.ipynb
